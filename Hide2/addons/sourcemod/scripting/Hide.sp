...
public void OnEntitySpawned(int iEntity, const char[] szClassname)
{
	if(iEntity > MaxClients && iEntity < 2048)
	{
		RequestFrame(OnEntitySpawnedNext, iEntity);
	}
}

public void OnEntitySpawnedNext(int iEntity)
{
	EntityParent[iEntity] = GetEntPropEnt(iEntity, Prop_Data, "m_pParent");
}
...
{
	return IsHidden[iClient][iEntity > MaxClients ? EntityParent[iEntity]:iEntity] ? Plugin_Handled:Plugin_Continue;
}
