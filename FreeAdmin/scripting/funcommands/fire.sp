...
bool PerformBurn(int client, int target, float seconds)
{
	if(CheckClientAction(client, ACTION_BURN))
	{
...
public int MenuHandler_Burn(Menu menu, MenuAction action, int param1, int param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		else if(PerformBurn(param1, target, 20.0))
		{
			char name[MAX_NAME_LENGTH];
...
public Action Command_Burn(int client, int args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "[SM] Usage: sm_burn <#userid|name>");
	...
	if(PerformBurn(client, iTarget, seconds))
	{
		GetClientName(iTarget, szTargetName, 64);
		ShowActivity2(client, "[SM] ", "%t", "Set target on fire", szTargetName);
	}
