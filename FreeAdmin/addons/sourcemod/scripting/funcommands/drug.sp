...
bool PerformDrug(int client, int target, int toggle)
{
	if(CheckClientAction(client, ACTION_DRUG))
	{
		switch (toggle)
		{
			case (2):
...
public int MenuHandler_Drug(Menu menu, MenuAction action, int param1, int param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		else if(PerformDrug(param1, target, 2))
		{
...
public Action Command_Drug(int client, int args)
{
	...
	if(PerformDrug(client, iTarget, toggle))
	{
		GetClientName(iTarget, szTargetName, 64);
		ShowActivity2(client, "[SM] ", "%t", "Toggled drug on target", szTargetName);
	}
	return Plugin_Handled;
}
