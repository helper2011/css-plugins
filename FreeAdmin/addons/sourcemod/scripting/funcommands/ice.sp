...
bool PerformFreeze(int client, int target, int time)
{
	if(CheckClientAction(client, ACTION_FREEZE))
	{
...
public int MenuHandler_Freeze(Menu menu, MenuAction action, int param1, int param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		else if(PerformFreeze(param1, target, g_Cvar_FreezeDuration.IntValue))
		{
			char name[MAX_NAME_LENGTH];
...
public Action Command_Freeze(int client, int args)
{
	if (args < 1)
	{
		ReplyToCommand(client, "[SM] Usage: sm_freeze <#userid|name>");
	...
	if(PerformFreeze(client, iTarget, seconds))
	{
		GetClientName(iTarget, szTargetName, 64);
		ShowActivity2(client, "[SM] ", "%t", "Froze target", szTargetName);
	}
