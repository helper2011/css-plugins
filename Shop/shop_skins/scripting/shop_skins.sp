	...
	if (KvGotoFirstSubKey(kv))
	{
		do
		{
			...
			if (Shop_StartItem(category_id, item))
			{
				...
				Shop_SetInfo(item_name, desc, hKeyValues.GetNum("price", 5000), hKeyValues.GetNum("sell_price", 2500), Item_Togglable, hKeyValues.GetNum("duration", 86400));
				Shop_SetCallbacks(_, OnEquipItem, _, OnItemDisplay);
				
				if (hKeyValues.JumpToKey("Attributes", false))
				{
					Shop_KvCopySubKeysCustomInfo(hKeyValues);
					hKeyValues.GoBack();
				}
...
ProcessPlayer(client)
{
	if (!client || selected_id[client] == INVALID_ITEM || IsFakeClient(client) || !IsPlayerAlive(client) || !IsValidClientTeam(client))
	{
		return;
	}
