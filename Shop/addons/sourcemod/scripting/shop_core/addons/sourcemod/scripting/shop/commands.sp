...
public Action Commands_Shop(int client, int args)
{
	...
	{
		PrintToChat2(client, "%t", "DataLoading");
...
public Action Commands_GiveCredits(int client, int args)
{
	...
	{
		PrintToChat2(client, "%t", "NoAccessToCommand");
		return Plugin_Handled;
	}
	char buffer[96];
	if (args < 2)
	...
	if (count < 1)
	{
		if (client)
		{
			PrintToChat2(client, "%t", "TargetNotFound", pattern);
		}
		else
		{
	...
	else
	{
		...
		if (client)
		{
			PrintToChat2(client, "%t", "give_credits_success", imoney, buffer);
		}
		else
		{
...
public Action Commands_TakeCredits(int client, int args)
{
	...
	else
	{
		...
		if (client)
		{
			PrintToChat2(client, "%t", "remove_credits_success", imoney, buffer);
		}
		else
		{
...
public Action Commands_SetCredits(int client, int args)
{
	...
	else
	{
		...
		if (client)
		{
			PrintToChat2(client, "%t", "set_credits_success", imoney, buffer);
		}
		else
		{
