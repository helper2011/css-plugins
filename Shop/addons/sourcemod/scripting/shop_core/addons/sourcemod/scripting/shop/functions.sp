...
bool Functions_ShowCreditsTransferMenu(int client)
{
	...
	if (!Functions_AddTargetsToMenu(menu, client))
	{
		...
		PrintToChat2(client, "%t", "no_targets");
		...
		case MenuAction_Select :
		{
			switch (param2)
			{
				case 1 :
				{
					...
					PrintToChat2(param1, "%t", "TransferSuccess", amount_give, target);
					PrintToChat2(target, "%t", "ReceiveSuccess", amount_give, param1);
					if (!g_bTransMode && amount_commission > 0)
					{
						PrintToChat2(target, "%t", "ReceiveCommission", amount_commission);
					}
...
Action Functions_OnClientSayCommand(int client, const char[] text)
{
	...
	if(g_iCreditsTransferAmount[client] < 2)
	{
		...
		PrintToChat2(client, "%t", "IncorrectCredits");
		return Plugin_Handled;
	}
...
void Functions_SetupLuck(int client)
{
	...
	PrintToChat2(client, "%t", "Lucker", category, item);
