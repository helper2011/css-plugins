			...
			switch (info[0])
			{
				case '0':
				{
					if (!ShowCategories(param1))
					{
						...
						PrintToChat2(param1, "%t", "EmptyShop");
					}
				}
				case '2':
				{
					...
					{
						ShowMainMenu(param1, GetMenuSelectionPosition());
						PrintToChat2(param1, "%t", "EmptyInventory");
					}
				}
				case '3':
				{
			...
			switch (iButton[param1][param2])
			{
				...
				case BUTTON_TRANSFER :
				{
					if (has)
					{
						...
						{
							PrintToChat2(param1, "%t", "no_targets");
							ShowItemInfo(param1, iClItemId[param1]);
						}
					}
					else
	...
	switch (action)
	{
		case MenuAction_Select :
		{
			...
			if (!target)
			{
				...
				PrintToChat2(param1, "%t", "target_left_game");
				return 0;
			}
			...
			if (type != Item_Finite && ClientHasItem(target, iClItemId[param1]))
			{
				...
				PrintToChat2(param1, "%t", "already_has", target);
				return 0;
			}
			if (!ClientHasItem(param1, iClItemId[param1]))
			{
				ShowItemInfo(param1, iClItemId[param1]);
				PrintToChat2(param1, "%t", "no_item");
				return 0;
			}
...
void ShowTransItemInfo(int client)
{
	...
	if (!target)
	{
		...
		PrintToChat2(client, "%t", "target_left_game");
		return;
	}
	if (!ClientHasItem(client, iClItemId[client]))
	{
		ShowItemInfo(client, iClItemId[client]);
		PrintToChat2(client, "%t", "no_item");
		return;
	}
		...
		case MenuAction_Select :
		{
			switch (param2)
			{
				case 1 :
				{
					...
					PrintToChat2(target, "%t", "receive_item", param1, category, item);
...
bool BuyItem(int client, int item_id, bool by_native)
{
	...
	if (GetCredits(client) < price)
	{
		if (!by_native)
		{
			PrintToChat2(client, "%t", "NotEnoughCredits");
		}
		return false;
	}
...
void OnAuthorized(int client)
{
	...
	if (g_sChatCommand[0])
	{
		PrintToChat2(client, "%t", "DataLoaded", g_sChatCommand);
	}
	else
	{
		PrintToChat2(client, "%t", "DataLoaded2");
	}
}
...
bool SetCredits(int client, int credits, int by_who)
{
	...
	if (by_who > 0)
	{
		PrintToChat2(client, "%t", "set_you_credits", credits);
	}
...
int RemoveCredits(int client, int credits, int by_who)
{
	...
	if (by_who > 0)
	{
		PrintToChat2(client, "%t", "take_you_credits", credits);
	}
...
int GiveCredits(int client, int credits, int by_who)
{
	...
	if (by_who > 0)
	{
		PrintToChat2(client, "%t", "give_you_credits", credits);
	}
...
void OnPlayerItemElapsed(int client, int item_id, bool notify = true)
{
	...
	if (notify)
	{
		PrintToChat2(client, "%t", "ItemElapsed", category, item);
	}
}
